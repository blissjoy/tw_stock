...
流程：抓該天 TWSE 全市場批次 + TPEx（透過FinMind逐股抓股價，實測約1小時內）資料 → 寫入Turso →
...
- **排程**：GitHub Actions，`.github/workflows/daily_pipeline.yml`，收盤後（台灣時間19:30）
  自動抓資料、選股、通知，整體實測約1小時內完成。TPEx 只抓~640檔4碼普通股股價（濾掉
  ETF/債券/權證，且法人/融資券目前沒有規則用到、不抓），`src/data/finmind_client.py`
  內建主動節流(_throttle)確保不會撞到FinMind每小時額度上限（實測發現超過額度不是優雅
  變慢而是直接回傳402、要等一小時視窗過去才恢復，因此節流比事後重試可靠）。
