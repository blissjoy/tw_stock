...
def backfill_twse(conn, start: date, end: date, sleep_sec: float = 0.6) -> dict:
...
def backfill_tpex(conn, stock_ids: list[str], start: date, end: date, sleep_sec: float = 0.3) -> dict:
    ...
    for stock_id in stock_ids:
        try:
            ...
            storage.upsert_stocks(conn, [{"stock_id": stock_id, "name": stock_id, "market": "TPEx", "industry": None, "updated_at": datetime.now().isoformat()}])
            if prices:
                storage.upsert_stock_prices(conn, prices)
            if institutional:
                storage.upsert_institutional_investors(conn, institutional)
...
def main() -> None:
    ...
    twse_results = backfill_twse(conn, start, end)
    ...
    if not args.skip_tpex:
        ...
        if args.tpex_limit:
            tpex_ids = tpex_ids[: args.tpex_limit]
        print(f"共{len(tpex_ids)}檔上櫃股票")

        print(f"\n=== TPEx 回補 {args.start} ~ {args.end} ===")
        tpex_results = backfill_tpex(conn, tpex_ids, start, end)
