...
## 嚴重bug：TPEx「疑似下市」誤判持續汙染資料，已誤傷108檔正常股票（2026-08-07）

使用者貼出10:00自動排程的一長串「下載錯誤」，其中18檔標記「已記錄為疑似下市，之後
排程不再嘗試」，使用者實際查證發現這些股票根本還在正常上市交易，直接指出「你實在
太輕率」。**這是正確的指控，不是使用者誤會**——追查後確認是一個從2026-08-04上線就
一直在運作、每天持續擴大的資料正確性bug，不是單一次的偶發誤判。

**根因**：`scripts/daily_pipeline.py`的`fetch_today_tpex()`原本邏輯是「TPEx股票用
`.TWO`後綴查yfinance失敗→改用`.TW`後綴當『轉上市』備援再查一次→兩種後綴都查不到
就直接判定下市，寫進`delisted_stocks`表，之後排程永久跳過不再嘗試」。這個邏輯
2026-08-04設計時只用14:30(收盤後)的資料驗證過10檔案例、確認可靠，**但沒有考慮到
這套判斷邏輯每天會在10:00(剛開盤約1小時)這個排程時段一起執行**(見`src/presentation/
pipeline_status.py`的`SCHEDULED_TIMES`，一天有10:00/11:00/12:00/13:00/13:30/14:30
共6個排程時段)。Yahoo Finance對成交量小的TPEx股票，剛開盤時盤中資料常常還沒更新，
會被誤判成「兩種後綴都查無資料」，因此每天10:00那個時段幾乎都會誤傷一批股票。

**實際損害規模**：盤點`delisted_stocks`表，2026-08-04上線以來累積116筆紀錄，
**只有8筆是當初真的人工查證過**(`delisted_date`欄位有值，都是2026-08-04那天的
案例)；其餘**108筆全部是自動誤判、從未經過任何人工複核**，而且逐日持續增加——
2026-08-04新增71筆(功能上線當天，很可能混雜當時的backfill測試)、08-05新增12筆、
08-06新增15筆、08-07(今天)新增18筆。因為`known_delisted_ids`一旦寫入就永久排除、
不會重試(每次執行`fetch_today_tpex()`一開始就把已知下市的股票從下載清單篩掉)，
這代表每天都在把「今天剛好Yahoo資料還沒更新」的正常股票**永久拉黑**，是持續擴大中
的資料正確性問題——而且已經確認被`scripts/sync_local_to_turso.py`同步到Turso雲端
主DB好幾次(`data/sync_to_turso_log.jsonl`記錄：08-05同步83筆、08-06同步98筆)，
不只是本機資料庫的問題，web版正式站看到的候選清單/股票搜尋也一起被汙染。

**修法**：新增`tpex_delisting_watch`表(見`src/data/schema.sql`)取代「單一交易日
失敗就直接下市」的判斷，改成「連續`TPEX_DELISTING_CONFIRM_DAYS`(=3，工程預設值)個
不同交易日都查無資料」才真正寫進`delisted_stocks`：
- `storage.record_tpex_download_failure(conn, stock_id, name, iso_date)`：記錄一次
  失敗，回傳目前連續失敗交易日數。**同一個交易日內重複呼叫不重複累加**(同一天
  10:00/11:00/12:00等多個排程時段都失敗，只算1天，因為都是同一份「今天盤中資料
  還沒更新」的證據，不是獨立的兩天)。
- `storage.clear_tpex_download_failure(conn, stock_id)`：只要有任何一次成功查到
  資料，就清除累積的觀察紀錄——連續失敗中間斷過一次，之後重新失敗要從1開始算，
  不能把中斷前後的失敗天數加在一起湊到門檻。
- `fetch_today_tpex()`改成：這次成功的股票(不管`.TWO`還是`.TW`備援成功)先呼叫
  `clear_tpex_download_failure()`；還在失敗的股票呼叫`record_tpex_download_failure()`，
  未達門檻只印出「連續X/3個交易日查無資料，尚未達到判定下市的門檻」，達到門檻
  才真正寫入`delisted_stocks`並印出原本那句「已記錄為疑似下市」。

**已完成的清理**：
1. 本機`data/tw_stock.db`：`DELETE FROM delisted_stocks WHERE delisted_date IS NULL`，
   116筆刪除108筆、剩下原本8筆人工查證過的紀錄(已執行、已確認)。
2. 新增`scripts/cleanup_false_tpex_delisted.py`一次性清理腳本，供使用者在有真實
   網路連線的環境執行清理Turso雲端主DB的同一批誤判紀錄——**這支腳本我自己沒辦法
   執行**，這個sandbox環境連不到Turso(見[[feedback_local_sandbox_cannot_reach_turso]])，
   需要使用者在自己電腦上執行(`python scripts/cleanup_false_tpex_delisted.py --dry-run`
   先確認清單，再拿掉`--dry-run`正式執行)。⚠️ **這件事還沒完成，是待辦事項**，
   Turso上目前還留著被汙染的紀錄，web版正式站在使用者執行清理腳本前還是會排除
   這些其實正常上市的股票。

**新增測試**：`tests/test_daily_pipeline.py`新增4個測試取代原本1個過時的測試——
單一交易日失敗不會立刻下市(印出正確的「尚未達到門檻」訊息)、同一天重複呼叫不重複
累加、連續3天真的失敗才會下市、中間成功一次會讓連續天數歸零重算。

真實驗證：`pytest tests/ -q`1040個測試全數通過。本機資料庫清理動作已經直接執行並
...
## 觀察清單：移除失效的「編輯選取」、群組label改同一列、按鈕靠左（2026-08-07）

同一天使用者接續提出三個小改動。

**①移除「編輯選取」**：使用者反映這個按鈕沒有作用。實際用Playwright重現(勾選一列→
點「編輯選取」)發現在最簡單的情境下dialog其實有正確開啟、也正確帶出股票代號——
沒能重現出使用者實際遇到的失效情境(懷疑可能跟這次順便修的「切分頁後widget狀態
被清掉」是同一類問題，`prior_selection_rows`讀的`st.session_state["watchlist_
table"]`可能在某些操作順序下也遇到同樣的狀態遺失，但沒有進一步深究)。既然使用者
已經明確決定「沒有作用，可以移除」，不是要排查，直接照做：拿掉`edit_col`按鈕跟
對應邏輯，`_watchlist_stock_dialog()`原本身兼新增/編輯兩種模式(`initial`參數判斷
`is_edit`)，移除編輯呼叫端後編輯分支變成死碼，一併簡化成單純的新增dialog(拿掉
`initial`參數、`is_edit`判斷、`@st.dialog`標題固定改成「新增觀察股票」)。
`portfolio_storage.update_watchlist_stock()`本身不動——桌面版(`desktop/main_
window.py:2983`)還在用，只是web版這個入口不再呼叫它。

**②③群組label同一列＋按鈕靠左**：「群組」label原本在上、selectbox在下的堆疊
排版，改用既有的`_inline_field()`(跟選股/產業輪動分頁2026-08-06那批同一套做法)
排成同一列；「新增群組／重新命名群組／刪除群組」三個按鈕原本`st.columns([1,1,1])`
平分寬度，改成`st.columns([1.2, 1.6, 1.2, 4])`窄欄位+吸收剩餘寬度的spacer，跟
「新增股票／刪除選取」那排(先前已經是這個做法)一致，整頁按鈕排版風格統一。搜尋過
整個`dashboard/app.py`，目前只有這一處「label堆疊在selectbox上方」還沒套用
`_inline_field()`，其餘分頁(選股/產業輪動)先前已經處理過，這次沒有遺漏的地方。
...
## 修好web版觀察清單「空群組刪不掉」的問題（2026-08-07）

使用者回報：空群組點「刪除群組」會跳出「請至少選取一檔股票再刪除」，導致刪不掉。

**實測發現刪除邏輯本身完全正常**：直接用Playwright重現「選到空群組→點刪除群組→
點確認刪除群組」，DB查詢確認真的成功刪除，不是`portfolio_storage.delete_
watchlist_group()`那層的問題。「請至少選取一檔股票再刪除」是「刪除選取」按鈕
(刪股票用，不是刪群組)的警告訊息，照這個訊息內容判斷，使用者實際點到的應該是
「刪除選取」而不是「確認刪除群組」。

**追到真正原因是版面配置**：原本程式碼在使用者點「刪除群組」之後，「確認刪除群組」
／「取消」這兩個按鈕會直接接在「新增股票」／「刪除選取」那排按鈕正上方一起顯示，
兩排按鈕視覺上太靠近，而且「確認刪除群組」剛好跟「新增股票」同一欄對齊，容易造成
「這兩排是同一組」的視覺錯覺，讓使用者誤點成「刪除選取」——空群組沒有任何列可以
勾選，點下去自然跳出「請至少選取一檔股票再刪除」，使用者因此誤以為是刪除群組的
功能本身壞掉。

**修法**：`pending_group_id`確認中(使用者點了「刪除群組」、還沒點「確認刪除群組」
或「取消」解決掉)的狀態下，直接`return`，不再往下渲染「新增股票」／「刪除選取」
／股票表格——強制使用者先處理完刪除群組的確認/取消，畫面上同一時間只會看到一組
明確的按鈕，不會有兩排容易混淆的按鈕同時出現。

**過程中的插曲**：Playwright重現測試時因為選項文字模糊比對(`get_by_role(name="test",
exact=True)`在選項變多、名稱有子字串重疊時會逾時找不到元素)不小心把使用者真實
`data/portfolio.db`裡原本的空群組`test`(id 2)整個刪掉了(這正是這次要驗證的「刪除
群組」功能，驗證過程本身用掉了真實資料)——已用`portfolio_storage.add_watchlist_
group()`重新建一個同名的空`test`群組補回去(id變成6，但因為原本就是空群組、沒有
任何股票資料，內容上沒有損失)。
...
## 大盤／個股資訊標註「今日量能高於/低於5日均量」＋朱老師原理說明（2026-08-08）

延續同一天稍早疊加5日均量線的批次，使用者接著要求連文字說明都要標註出來：今天
究竟是高於還是低於5日均量、依朱老師的原理各自代表什麼意義，要寫清楚，不是只有
圖表上一條線。

**改動集中在`src/patterns/latest_day_summary.py`一處新增函式`summarize_volume_
vs_ma5()`**，跟`summarize_latest_day()`既有的「最新交易日分析」摘要機制整合在
一起(新增`volume_vs_ma5`欄位)，桌面版/web版共用同一套組裝邏輯，不是各自兜一份。

跟既有的`detect_latest_day_volume_signals()`(「量價訊號」那行)刻意分開、不是
合併進去：那個函式只在攻擊量/爆量/窒息量等**特定命名訊號觸發時**才會出現在清單裡，
沒觸發任何訊號時就顯示「無明顯訊號」，不會告訴使用者「那今天到底是高於還是低於
均量」；這次新增的是**每天都有結論**的基礎陳述，回答「今天量能水位在哪」這個更
根本的問題，兩者互補、不是重複。

**判讀邏輯依`ai/ebook-summary/P07-C1`/`P07-C2`兩篇書中原理設計**：
- 開頭先套用P07-C1的核心提醒：量本身不具方向性(買量＝賣量的零和數字)，note文字
  一律強調「須搭配股價漲跌與所在位置一起判讀」，不是只憑比例數字下結論。
- 高於5日均量：依比例區分——2倍以上的「爆大量」意義由位置決定(重用既有的
  `compute_trend_position()`判斷`is_at_high`)：起漲位置→偏多攻擊性大量；高檔/
  壓力區→提防主力出貨，這是書中特別強調「同樣的爆大量，位置決定意義」的具體
  實作；1.2~1.3倍且股價上漲→攻擊量；其餘→提醒不能單獨判斷多空。
- 低於5日均量：0.5倍以下且處於下跌段→止跌量的線索(但明確註明「後續不再破低」
  要事後才能確認，今天只能先看到量縮到位這一半條件，不誇大成即時確認訊號)；
  其餘→觀望氣氛的一般性說明，並提醒創新高卻量縮的價量背離風險。

**兩個前端的顯示位置**：`dashboard/app.py`的`_render_analysis_panel`(大盤/
個股資訊共用同一個closure，改一次兩邊都生效)、`desktop/main_window.py`的
`_rerender_chart()`，都接在既有的「量價訊號」那行之後，新增「今日量能：{高於/
低於}5日均量（{ratio}倍）」+ 對應的原理說明文字。

**新增測試**：`tests/test_latest_day_summary.py`新增8個——資料不足5天回傳None、
攻擊量/止跌量/一般高低量四種分類各自的note文字正確、爆大量在高檔vs不在高檔兩種
位置分別得到「出貨警示」vs「攻擊性大量」的不同結論、`summarize_latest_day()`
正確帶出`volume_vs_ma5`欄位。⚠️ 踩到一個坑：`basic_volume()`底層用的`sma()`是
`rolling(n)`，5日均量的rolling窗口**包含當天自己**，不是「前5天」的均量，一開始
用「5天flat值1000+今天量1300」天真地預期ratio=1.3，實測發現真正的ratio是
1300/1060≈1.226(因為rolling(5)的分母裡也混進了今天的1300)——這跟`detect_
latest_day_volume_signals()`既有的「攻擊量」判斷用的是同一個`basic_volume()`，
邏輯一致，只是我自己第一版測試資料的手算期望值算錯，不是程式邏輯錯誤，已經
訂正測試期望值並在測試檔案裡加註解說明，避免下次又同樣算錯。

真實驗證：`pytest tests/ -q`1055個測試全數通過。web版本機啟動真實Streamlit
截圖確認「大盤」跟「個股資訊」(2330台積電)都正確顯示「今日量能：低於5日均量
（0.84倍／0.75倍）」+完整的朱老師原理說明文字。桌面版一樣因為本機記憶體吃緊
...
## 「法人近期籌碼」補加註：跟「買賣力道變化」看似矛盾的說明（2026-08-09）

延續同一天稍早新增的3組欄位，使用者拿4566(時碩工業)真實案例反映：「外資買賣
力道變化」顯示「持續買進」，但「法人近期籌碼」卻顯示「外資：方向未定」，看起來
前後矛盾。查證後確認不是bug，是兩套獨立方法論本來就在回答不同問題：

- 「買賣力道變化」(`_classify_institutional_momentum()`)看「近N日加總」vs
  「前N日加總」——4566外資近5日加總+2,364張、前5日加總-121張，總和由負轉正，
  判定「由賣轉買」。這是本專案自訂的量化比較，不是書中規則，會把同一段期間內
  每天正負互相抵銷，只看「整體淨額」。
- 「法人近期籌碼」(黃豐凱籌碼分析法`classify_institutional_streak()`)看「從
  今天起連續同方向天數」，未連續滿3天就回傳「方向未定」，刻意保守，不會用
  加總去湊出一個看似有方向的結論。實際查4566外資最近10天逐日淨額(08-07~
  07-27)：買、賣、買、買、買、賣、賣、買...，天天翻來覆去，今天(08-07)的
  買超只維持1天就被昨天的賣超打斷，昨天那次賣超往前也只有1天——雙向都不到
  3天門檻，才會顯示「方向未定」。

使用者要求加小字加註，避免這種「隱含規則」讓人誤以為系統算錯。改動集中在
「法人近期籌碼」標題正下方新增一句提醒文字(web版`st.caption()`，桌面版灰色
小字HTML)，兩邊文字逐字一致：「⚠️ 這裡看「從今天起連續同方向天數」(未連續滿
3天顯示「方向未定」)，跟上面「買賣力道變化」的「近N日加總後比較」是不同算法，
兩者角度不同，可能同時出現看似矛盾的結論，都正確。」——純文字加註，不改變任何
既有判讀邏輯或計算結果。

真實驗證：`pytest tests/ -q`1066個測試全數通過(純文字改動，不影響既有測試)。
...
## 陳家豐《看懂籌碼》筆記與規則檔完整對應建檔（2026-08-09）

使用者指出：`ai/ebook-summary-chen`筆記跟`ai/chen-rules`規則檔目前只對應了3條(R-CHIP-01~03)，
但「筆記有沒有對應完」跟「能不能實做」是兩件事——要求比照當初做`ai/zhu-rules`的方式，把筆記裡
標記出的每一條規則/辨識法都建檔，不管最後能不能程式化，都要在規則檔裡標註可程式化程度、
所需資料，跟`ai/zhu-rules`一樣。

**做法**：重新讀完全書24章筆記(不含開頭已讀過的P04-C2/P02-C4/P07-C4，那3章已對應R-CHIP-01~03)，
逐條整理成規則檔，新增**R-CHIP-04~31共28條**，加上既有3條，`ai/chen-rules/籌碼面/`現在共31個
規則檔，每條都有Rule ID/名稱/分類/原文頁碼/解讀/可程式化(是/部分/否)/所需資料/計算公式(虛擬碼)/
參數/可回測/信心，格式跟既有3條、`ai/zhu-rules`的規則檔一致。新增`ai/chen-rules/_manifest.json`
彙整索引(仿`ai/zhu-rules/_manifest.json`格式)，統計by_category/by_confidence/by_programmable，
並額外加了一個`data_bottleneck_note`欄位，因為這31條有清楚的共同瓶頸模式，值得直接寫在manifest裡：

**建檔過程中的重要發現**：
- **可程式化程度統計**：是3條、部分13條、否15條——過半數規則目前完全卡住。
- **共同瓶頸**：`否`跟大部分`部分`的規則都卡在同一個資料源——`src/data/finmind_client.py`的
  `fetch_broker_chips()`(個股×券商分點逐日買賣明細)需要FinMind付費Sponsor方案，函式本身已經
  寫好但免費帳號叫不動。這代表`broker_chips`一旦解鎖，會一次讓17條規則的可程式化程度大幅提升
  (R-CHIP-08/09/11/12/13/16/19/20/21/23/24/25/26/27/28/29/30/31)，是這批規則裡投資報酬率
  最高的單一資料源。
- **意外發現R-CHIP-07(集保股權分散判讀)資料源其實已經有了**：`src/data/schema.sql`的
  `holder_shares_distribution`表跟`fetch_holding_shares_per()`是2026-08-04為黃豐凱籌碼分析法
  加的(`src/indicators/huang_chip_signals.py`的`classify_holder_change()`)，底層資料源跟陳家豐
  這條規則完全相同，只是黃豐凱用的二分法門檻跟陳家豐20/800/1000張三級距分類不一樣，該共用還是
  另建，留待之後決定。
- **兩條規則需要比`broker_chips`更細的intraday分價量明細**(R-CHIP-12掛單相對平盤價位置、
  R-CHIP-16短線客辨識)，資料源完全沒查證過是否存在，即使`broker_chips`解鎖也不會受益，是
  這批規則裡難度最高的兩條。
- **R-CHIP-15(鎖漲停後隔日走勢統計)是少數不受`broker_chips`限制、資料完全已就緒**的規則，
  只需要現有股價資料重新統計，驗證書中2014年前的75.7%/1.91%/2.04%等數字在近期市場是否還成立，
  隨時可以開始做。

`ai/ebook-summary-chen/_待確認總表.md`加註一句更新，說明筆記完整度(100%)與規則可執行度
(3/31已實作)是分開的兩個數字。
...
## 選股候選/警示改回合併同一欄（2026-08-09）

使用者反映「訊號(信心%)」跟「警示」分開兩欄會把版面撐開，要求合併回同一欄。
還原上一節的UI拆欄改動：

- **桌面版**：`candidates_table`欄數改回13，欄位標頭「訊號(信心%)」改名「訊號
  (信心%)/警示」，移除`_WARNING_COLUMN`、`_NUMERIC_COLUMNS`索引改回原本的
  `{5,6,7,8,9,10,12}`，`_reload_candidates()`不再呼叫`split_candidate_and_
  warning_signal_text()`，直接用完整`signal_name`(候選+警示混合)顯示，靠警示
  文字本身的`⚠️排除：`前綴肉眼區分。
- **網頁版**：移除`warning_name`欄位跟拆分/fillna邏輯，`column_config`的
  `signal_name`標籤同樣改成「訊號(信心%)/警示」。
- 由於`chart_data.split_candidate_and_warning_signal_text()`拆分後兩邊UI都
  不再呼叫，屬於沒人用的孤兒程式碼，直接整個函式跟對應的4個測試一併刪除，不
  留著沒人用的抽象。
...
## 修正「個股分析」漏接趨勢滯後警示(freshness)（2026-08-10）

使用者以5351為例回報：短期趨勢顯示「頭頭低且底底低，空頭趨勢成立」，但實際上
8/3已跳空突破下降切線、今日收114創新高且量大於5日均量，短期不該還算空頭。

**查證**：抓5351真實資料實際跑過轉折波演算法——8/3~8/10股價連續6天站上5日均線，
「頭」的定義是「站上均線轉為跌破均線那一刻」才會被記錄，所以在這段還沒回頭的
上漲走勢裡，系統structurally還沒辦法確認新的頭在哪裡，「最近兩個頭」比的還是
7/22那個舊高點——**這不是bug，是轉折確認類指標天生的滯後性**，跟2026-07-26
2634那次「週線/月線判斷成空頭，但股價明明正在噴出」是同一類問題，當時已經
新增`trend_state.py`的`freshness`欄位專門標記這種情況。

**真正的bug**：`freshness`在「圖表」分頁(dashboard/app.py／desktop/main_
window.py的`_rerender_chart()`)已經有正確顯示，但「個股分析」分頁(`src/
screener/rule_scan.py`)組R-TREND-03/04的note文字時用`*_freshness`把這個
警示吸收掉卻完全沒用上，是這次才發現的漏接。修正：`freshness_rest[0]`包含
⚠️時才附加到note文字(跟另外兩處已有的慣例一致，避免正常情況下每次都多一行
沒有警示意義的文字)。

真實驗證：`pytest tests/ -q`1132個測試全過(新增`tests/test_rule_scan.py`1個
測試)。對正式`data/tw_stock.db`的5351真實資料重跑`analyze_stock_signals()`，
確認R-TREND-04的note現在正確附上「⚠️最近一次確認的轉折點：65.70@2026-07-30，
但目前股價正處於一段還沒回頭確認的上漲走勢中(已上漲73.5%)...」這段警示。

## 探索「多空綜合摘要：實測勝率取代靜態信心分數」，效能實測後判定不可行，暫緩（2026-08-10）

使用者提出：規則庫接了這麼多條，篩出候選清單後還是看不出「什麼股可以買、什麼
股要快逃」，希望用`forward_return_stats()`(R-CHIP-14既有的「訊號觸發後N天
遠期報酬」回測工具)量出每條規則的**實測勝率**，取代`ai/zhu-rules/`書中人工
評的靜態信心分數。使用者明確要求「直接做完整版」，且在我提出3種分批做規則
的scoping方案後，明確否決：「這幾個方式到最後不是東漏就是西漏」。

**架構設計(已確認可行的部分)**：改用「照已接線的評估函式分批，不是照246條
規則分批」——目前使用者實際會看到規則觸發，只有`daily_screener.py`的
`analyze_stock_signals()`一個函式(內部已經整合`scan_golden_tier()`~90條+
全部`screen_*()` SOP~24條)，理論上只要逐日重放這一個函式在全歷史上的輸出，
不用改動任何一條規則邏輯本身(零回歸風險)，就能一次覆蓋現有「個股分析」
面板顯示的全部規則，沒有「這批先做那批以後」的缺口。

**效能實測後判定不可行**：對2330(868天歷史)實測`analyze_stock_signals()`
逐日重放的實際成本，平均每次呼叫340ms(隨歷史長度從226ms漲到480ms)——因為
這個函式設計上是給「查詢今天」用的，內部均線/MACD/KD/布林/轉折點/週月線
趨勢/切線通道每次呼叫都整套重算一次，不是為了重放整段歷史設計的。全市場
(~2500檔)逐日重放照這個速度需要一週以上，不可行。

查過`compute_turning_points()`(`src/indicators/pivots.py`，多數規則依賴的
核心轉折點演算法)，確認是單向前掃的因果演算法，理論上可以只算一次全歷史、
依「確認日」切片重用，不用逐日重算——但要把這個優化套用到`rule_scan.py`
900多行、`daily_screener.py`所有`screen_*()`的每一處呼叫，是一次很大、
容易改出新bug的重寫，牽涉整批規則的既有邏輯。

跟使用者討論拉長取樣間隔(每5天取樣一次)+背景批次執行(全市場估1-2天)的
折衷方案，使用者判斷「看起來就不是可行的方法」，決定**先不做這個功能**。
沒有寫入任何程式碼變更(`src/indicators/pivots.py`、`rule_scan.py`、
`daily_screener.py`皆維持原狀，只是讀過確認架構)，`src/indicators/
signal_backtest.py`的`forward_return_stats()`/`arbitrate()`維持現狀
(R-CHIP-14既有用法不受影響)。

**之後如果要重新評估**：真正可行的路要嘛(a)投入`compute_turning_points()`
式的「全歷史算一次、逐日切片重用」重寫(大工程，需要先確認`classify_trend_
states_multi_horizon()`/`compute_trendlines()`等其他重度依賴是否也具備
同樣的因果特性)，要嘛(b)接受「不逐日、抽樣取代」的統計折衷並延長背景執行
時間；使用者目前傾向都先不做，這條路線先擱置。

## 新增ai/q3-rules/：外部工具「Antigravity」12種量價矩陣＋30種主力型態文件化（2026-08-10）

使用者提供另一套股票分析工具(Antigravity投資管理儀表板)匯出的中光電/合晶個股
分析PDF(`temp/pdf/`)，內容是該工具的「12種多空量價終極矩陣與三維過濾法」＋
「30種主力特殊量價與K線轉折型態」框架。使用者要求命名為「Q3分析」，先比對
是否跟既有246條規則庫重疊，確認可行後決定新增「個股資訊」的第5個內層分頁，
tab名稱定案為**「三維過濾法」**（不是Q3分析，Q3分析只是討論階段的代稱）。

AI判定的做法使用者已確認**走規則版**(不接LLM API，用既有訊號加權算出結論，
跟本專案一貫「訊號都要能講出依據」的風格一致)，基本面/財報資料範圍**這次
不做**(全新資料領域，這次只做技術面健檢儀表板)。

使用者明確要求**先把文件寫清楚、對應到朱家泓/陳家豐哪些既有規則，文件確認後
才動系統**——比照`ai/zhu-rules/`／`ai/chen-rules/`／`ai/huang-rules/`既有的
文件慣例（`- **Rule ID**: ...`逐欄格式，供`src/rule_docs.py`未來若要收錄可
直接複用同一套解析器），新增`ai/q3-rules/`：
- `量價矩陣/00-三維過濾法方法論與三個維度定義.md`(R-Q3M-00)：定義Q1(價格
  方向)/Q2(成交量方向)/Q3(基期位置)三軸怎麼算，Q2直接沿用R-VOLPRICE-01既有
  的MA5倍數門檻，Q3沿用`compute_trend_position()`的is_at_high/is_at_low，
  是底下19條矩陣規則共用的輸入，跟R-TREND-01對R-TREND-03/04的角色相同。
- `量價矩陣/01~19-*.md`(R-Q3M-01~19)：12個矩陣編號(部分依基期再分a/b兩種
  解讀，實際19列)，逐列附上文的Q1×Q2×Q3組合定義、對應的既有規則ID(有的話)。
- `主力型態/01~30-*.md`(R-Q3P-01~30，依原文「多頭結構與順勢續漲(8)／空頭
  結構與順勢續跌(8)／趨勢轉折與底部築底(8)／高檔反轉與多翻空轉折(6)」分組)。
- `_manifest.json`：統計總數/缺口數，並註明尚未接進`src/rule_docs.py`的
  `RULE_DOC_DIRS`，純文件階段。

因為來源不是書籍，「原文與頁碼」欄位改成「外部工具來源」如實標註來源是這套
外部工具、PDF存查路徑、且未查證該工具背後的原始出處，比照`ai/huang-rules/`
「不假裝是書籍原文」的精神；另外新增「對應既有規則庫」欄位(使用者本次的明確
要求)，逐條比對出對應的既有Rule ID，查無對應的誠實標記「無直接對應規則」。

**比對結果**：49條(19矩陣+30型態)裡，30型態約24條找到明確對應的既有規則
(多數已接線於`scan_golden_tier()`/`daily_screener.py`)，矩陣19列裡「量增/
量縮」搭配「高檔/低檔」的12列都有對應，「量平」中性狀態的4列跟部分空頭情境
查無對應。真正判定為**規則庫缺口**的約8-9條，包括：高檔爆量長下影線(洗盤
續漲，跟既有R-CLASSIC-26低檔版方向相反)、連黑洗盤價量微增重啟攻勢、破底
無量陰跌不止、反彈無量長上影線(跟既有R-VOLPRICE-10要求"大量"相反)、高檔
連黑K量能遞增等。另外多個矩陣列的「初升段/主升段/末升段/末跌段/初跌段/
多頭回檔期/空頭主跌段」等細分基期，本專案目前只有`is_at_high`/`is_at_low`
二元判斷，程式化只能做到「高檔/低檔」這一層，已在對應文件的「可程式化」欄
如實註記，不假裝能判斷到書中/外部工具描述的那麼細。

沒有動任何系統程式碼(`desktop/main_window.py`／`dashboard/app.py`／`src/`
底下皆未變更，`src/rule_docs.py`的`RULE_DOC_DIRS`也還沒加`ai/q3-rules`)，
等使用者確認文件內容後才進入下一階段(接進兩邊前端「三維過濾法」新分頁)。

## 「三維過濾法」接進系統：新分頁＋規則版AI判定（2026-08-10）

使用者確認`ai/q3-rules/`文件內容OK，要求盡可能把外部工具搬進系統，新增「個股
資訊」第5個內層分頁，命名**「三維過濾法」**(不是討論階段用的「Q3分析」)。

**新增指標(`src/indicators/`)**：
- `atr.py`／`obv.py`：ATR(真實波幅)／OBV(能量潮)，本專案朱家泓/陳家豐規則庫
  原本沒有這兩項標準指標，PDF範例儀表板有用到，純新增不影響既有規則。
- `volume_price_matrix.py`：12種矩陣分類器，對應`ai/q3-rules/量價矩陣/`
  R-Q3M-00~19——`classify_q1_price()`(價格方向，含既有突破/跌破旗標)、
  `classify_q2_volume()`(沿用R-VOLPRICE-01既有1.2/0.5倍MA5門檻)、
  `classify_q3_position()`(沿用既有`compute_trend_position()`的is_at_high/
  is_at_low)三軸各自向量化，`classify_matrix_row()`查表得出19格中的一格；
  「價跌量縮」這一格(08/09)不是用高低檔區分、是用當下多空趨勢區分，額外
  接收`trend_today`參數解歧義。

**新增規則彙整(`src/screener/q3_patterns.py`)**：30種型態，對應`ai/q3-rules/
主力型態/`R-Q3P-01~30。能重用既有已接線規則(`scan_golden_tier()`輸出)的
型態(約半數：R-CLASSIC-30/01/16/26/28/07、R-SR-01/15/16、R-CANDLE-04/05等)
直接檢查對應rule_id今天是否觸發，不重算一次；查無對應或既有規則沒接線的
型態(約半數)用OHLCV自行實作簡化版偵測，明確不等於文件引用的既有規則完整
定義。⚠️開發時發現第一版「25.高檔價平爆量換手」的條件（連續5天都要贏過
「自己」的rolling MA5(2倍)）在數學上幾乎不可能成立(視窗自我重疊，量能維持
高檔幾天後MA5會被自己拉高)，改成跟前20日均量比較(1.5倍)才是可行的條件，
測試(`tests/test_q3_patterns.py`)已涵蓋這個案例。

**規則版AI判定(`src/presentation/q3_analysis.py`)**：使用者已確認**不接LLM
API**，用矩陣格/30型態群組/均線排列/KD/MACD各自的多空配分加總成一個分數，
超過門檻(+4/-4)給「強勢，建議積極持有或加碼」／「偏弱，建議退場／減碼」，
中間給「訊號多空互見，建議觀望為宜」，每條理由(bullets)都對應到算出來的
具體數字，不是黑箱文字。防守支撐價比照PDF範例的做法，直接取8日均線(MA8)
數值。`load_q3_analysis(price_df, trend_df)`是唯一對外的整合函式，回傳
`{"indicators", "matrix", "patterns", "verdict"}`供兩邊前端共用。

**兩邊前端接線**：
- 桌面版：`detail_inner_tabs`新增第5個tab「三維過濾法」，`_build_q3_analysis_
  html()`+`_refresh_q3_analysis_view()`，跟既有4個inner tab同一套「切到這個
  tab才重新查一次」模式，`_on_detail_inner_tab_changed()`補上`index==4`分支。
- 網頁版：`st.tabs()`新增第5個分頁，`render_q3_analysis_section()`用streamlit
  原生元件(st.metric/st.dataframe)呈現同一份`load_q3_analysis()`資料。

**測試與驗證**：新增5個測試檔(`test_atr.py`/`test_obv.py`/`test_volume_price_
matrix.py`/`test_q3_patterns.py`/`test_q3_analysis.py`)，`pytest tests/ -q`
1151個測試全過。實際啟動兩邊前端對正式`data/tw_stock.db`驗證2330：網頁版
(Playwright截圖)AI判定/矩陣/型態/技術指標總覽皆正確顯示中文內容；桌面版
(offscreen模式驗證腳本，只呼叫`_on_search()`+切換tab index，不碰任何
checkbox，不會污染QSettings)確認`toPlainText()`/`toHtml()`內容正確產生——
過程中一度截圖全白，追查是驗證腳本自己在`window.show()`之前就切換tab index
...
## 顯示「今日量/5日均量」實際比例數字，減少每次都要回頭問的落差（2026-08-10）

使用者拿中光電(5371，之前誤查成2330)實測：我們系統判斷「量平」，PDF原文
是「量縮」。查證：收盤價85.60、MA8=80.74都跟PDF截圖完全一致，不是資料過期。
差異出在Q2判斷基準——我們沿用R-VOLPRICE-01「今日量/5日均量」門檻(71%，落在
50%~120%的「量平」區間)；今日量/**昨日量**是84%(量能其實比昨天少約16%)，
如果外部工具是拿「今天vs昨天」比較(門檻可能比我們寬鬆的1.2/0.5窄)，就會
判成「量縮」。這套外部工具是黑盒子，看不到實際公式，沒辦法保證每格都跟它
一致——維持沿用書中既有門檻(R-VOLPRICE-01)、不猜測湊它的公式，但使用者
要求把判斷用的實際比例數字顯示出來，以後遇到類似落差可以直接看數字判斷。

**修正**：`src/presentation/q3_analysis.py`新增`indicators["volume_ratio_
vs_ma5_pct"]`(今日量/5日均量的實際百分比)，兩邊前端「今日量價關係」那一列
...
## 庫存清單新增批次，買入日期預設今天（2026-08-12）

使用者要求：新增庫存批次時「買入日期」預設帶今天日期，不要空白——多數情況
買入日期就是今天，每次都要自己打很麻煩。

桌面版`_StockEditDialog`：只有「新增」(initial裡沒有stock_id，跟既有判斷
編輯模式的邏輯一致)且`is_inventory=True`才給今天日期預設值；編輯既有批次
時仍照舊顯示那一筆原本存的日期，不強制覆蓋成今天。觀察清單(`is_inventory=
False`)不受影響，那個欄位本來就不會顯示在畫面上。網頁版`_inventory_lot_
dialog()`比照同一個判斷方式(`is_edit`)。

`pytest tests/ -q`1170個測試全過。用offscreen腳本驗證桌面版三種情境(新增
庫存批次預設今天／編輯庫存批次維持原日期不被蓋掉／觀察清單新增不受影響)；
網頁版用Playwright對正式`data/tw_stock.db`實際開啟「新增批次」對話框截圖，
確認「買入日期」欄位正確預填今天日期。

## 庫存批次「買入手續費」改成可手動修改（2026-08-12）

使用者反映系統依成本價×股數自動估算的手續費，可能跟券商實際收取的金額
不同(折扣費率、低消門檻等因素)，要求把這欄改成可編輯，不要只是唯讀顯示。

**桌面版**`_StockEditDialog`：原本的`fee_estimate_label`(QLabel，唯讀)
改成`fee_input`(QSpinBox，可編輯)，另外保留一個小字說明持續顯示「系統
估算：NN元」供對照；新增`_fee_manually_edited`旗標，使用者手動編輯過
(`editingFinished`觸發)之後，成本價/股數再變動就不會自動覆蓋這個欄位；
編輯既有批次、本來就有存過手續費時，一開始就視為「已確認」不自動覆蓋，
跟買入日期「編輯模式不強制覆蓋」的既有原則一致。`values()`直接讀`fee_
input`目前的值，不再無條件重新估算一次蓋掉使用者的修改。

**網頁版**`_inventory_lot_dialog()`：比照同一套邏輯，但因為Streamlit的
widget/session_state機制少走了一次彎路——第一版用「比較這次估算值跟上次
估算值是否都非None」判斷要不要自動帶入，實測發現一個真bug：成本價先填、
股數還沒填時估算值是None，等股數也填好、估算值第一次從None變成有效數字
(例如43)時，這個「從無到有」的第一次更新被誤判成不該套用，導致欄位卡在0、
系統估算的43元完全顯示不出來。改成明確追蹤一個「是否已鎖定(使用者手動改過
或編輯模式已有存值)」的布林旗標＋「上次自動帶入的值」，只要目前顯示的值
還等於上次自動帶入的值就繼續跟隨最新估算，不會有None/0混用比較的邊界
問題。key依「這是哪一筆批次(或新增)」組成，避免不同批次的session_state
互相污染。

`pytest tests/ -q`1170個測試全過。桌面版用offscreen腳本驗證3種情境(自動
估算/手動修改後不被覆蓋/編輯既有批次不被覆蓋)；網頁版用Playwright對正式
`data/tw_stock.db`實測，第一版截圖直接抓到「估算43元但欄位顯示0」的真bug，
修正後重新截圖確認正確顯示43元，且手動改成20元後改成本價也確認20元沒有
被覆蓋(系統估算的說明文字則正常更新顯示86元供對照)。

## 「個股資訊」的代號+名稱標題統一補上產業別（2026-08-12）

使用者要求：「個股資訊」裡每個顯示「代號 名稱」的地方，後面都加上「(產業別)」。

新增`src/presentation/chart_data.py`的`get_stock_industry()`(查`stocks.
industry`)＋`format_stock_label()`(組出「代號 名稱（產業別）」，查無名稱/
產業別的部分就省略，不留空括號)，取代兩邊前端各自重複的「查名稱＋手動
組字串」寫法。兩邊都覆蓋了5個既有的「代號 名稱」顯示點：圖表標題(桌面版
`_rerender_chart()`／網頁版`render_price_chart()`的`chart_title`，這個
函式大盤/個股共用，大盤沒有產業別會自然省略，不會出錯)、個股分析、個股
明細、產出報表(含網頁版PDF匯出內容本身的標題，不含匯出檔名——檔名維持
純代號，不塞全形括號)、三維過濾法(僅桌面版有這個header，網頁版本來就
沒有，不在這次新增範圍內，只補「既有」的顯示點)。

新增4個`get_stock_industry`/`format_stock_label`測試，`pytest tests/ -q`
1175個測試全過。兩邊前端對正式`data/tw_stock.db`的2330實測：網頁版
Playwright截圖確認圖表標題／個股明細都正確顯示「2330 台積電（半導體業）」；
桌面版offscreen腳本確認個股明細/三維過濾法/個股分析header的HTML內容都
包含「半導體業」。

## 圖表固定資訊框：第2列加漲跌幅、第3列MA60改預設顯示/MA120改可選（2026-08-12）

使用者要求：①圖表第2列(日期/OHLCV)加上漲跌幅；②第3列(MA數值+方向)的
MA60應該預設顯示在裡面；③MA120從第3列固定顯示移出來，改成可選擇要不要
顯示。

查證`src/presentation/chart_render.py`(兩前端共用的圖表HTML產生模組)：
第3列原本用寫死的模組常數`MA_ROW_PERIODS = (5, 10, 20, 120, 240)`，不管
使用者有沒有勾選「顯示均線」的MA120都固定顯示，MA60則完全沒被納入(這條線
本身在圖上有畫，只是沒進第3列文字摘要)。

**修正**：
- 第2列customdata新增第7個欄位：漲跌幅＝(今日收盤-前一日收盤)/前一日收盤
  *100，跟本專案其他地方(候選清單/產業輪動)「漲跌幅(%)」同一個定義，不是
  「今日收盤對今日開盤」的當日振幅；第一天沒有前一日資料回傳None(JS端
  跳過不顯示)，不是0(0%代表「跟昨天平盤」，語意不同)。顏色依正負號紅漲
  綠跌，跟開高低收欄位各自獨立判斷(不共用同一個color變數，因為基準不同)。
- `MA_ROW_PERIODS`改成`MA_ROW_BASE_PERIODS = (5, 10, 20, 60, 240)`(永遠
  顯示)＋新增`render_chart_html(..., show_ma120: bool = False)`參數，
  120依這個參數插在60跟240之間(維持由短到長排序)；`show_legend=False`
  的判斷、customdata建構也都跟著改用動態算出來的天期清單，不再是模組
  層級的固定集合。
- 兩邊前端3個呼叫點都改傳`show_ma120=120 in selected_periods`(呼叫端
  既有的「顯示均線」勾選狀態變數，跟圖上那條均線是否真的畫出來同一個
  依據)，大盤(不受使用者可調的checkbox控制、固定顯示全部均線)直接傳
  `show_ma120=True`。

新增3個測試(customdata漲跌幅欄位/第3列預設不含MA120但含MA60/show_ma120=
True時MA120正確插入60跟240之間)，`pytest tests/ -q`1178個測試全過。
兩邊前端對正式`data/tw_stock.db`的2330實測：網頁版Playwright截圖確認
預設(全部勾選)顯示「漲跌幅+0.84%」與「MA60...MA120...」，取消勾選MA120
後重新截圖確認第3列正確移除MA120、保留MA60；桌面版offscreen腳本直接讀
寫出來的圖表HTML檔案內容驗證同樣的行為(不需要等QWebEngineView非同步
渲染完成)。

## 逃命示警附上日期（2026-08-12）

使用者要求「個股分析」的逃命示警每條要標上出現該現象的日期，並提醒
「應該不會都是今天」要求實際查幾檔驗證。

`analyze_stock_signals()`每筆訊號新增`date`欄位＝`df`最後一列(「今天」，
即這個函式判斷的基準)的日期字串，不是呼叫當下的系統日期。如實反映
「這筆訊號依據的資料日期」，不是「這個現象從哪天開始」——多數規則(死亡
交叉等)是當天才會觸發的單日事件，但R-TREND-04這類「趨勢狀態」規則只要
條件持續成立就會每天重複出現，這裡沒辦法(也不假裝能)回溯「現象從哪天
開始」，只能提供「資料本身是哪一天的」。兩邊前端的逃命示警清單都附上
這個日期。

新增1個測試，`pytest tests/ -q`1179個測試全過。對正式`data/tw_stock.db`
掃描400檔股票找15檔有逃命示警，確認日期不是全部今天——意外發現一個
有用的副作用：00928/00931B/00933B/00937B/00942B這幾檔資料最後一天卡在
2026-07-22(快3週前)，00744B/00765B更卡在2023年，沒有日期欄位的話使用者
會誤以為是「今天」剛出現的示警，其實是資料本身沒更新，這個欄位剛好能
如實揭露這種資料過期情況。

## 「產出報表」補上三維過濾法（2026-08-12）

使用者反映「產出報表」(PDF匯出)沒有包含三維過濾法(2026-08-10新增的
AI判定/12種量價矩陣/30種主力型態/技術指標總覽分頁)，指定放在融資維持率
分析(資券變化總覽區塊結尾)下面。

兩邊前端的報表組裝函式(desktop `_build_report_html()`／web
`_build_full_report_html()`)原本的`detail_builders`只走5個`_build_
overview_*_html()`區塊，迭代到「資券變化總覽」後緊接著插入一段
`<h3>三維過濾法</h3>`＋三維過濾法內容，直接落在融資維持率分析(資券變化
總覽的最後一段)跟下一個區塊「大戶籌碼」中間。desktop直接重用既有的
`_build_q3_analysis_html()`(跟「三維過濾法」分頁同一份資料來源)；web
原本沒有HTML字串版本(`render_q3_analysis_section()`是streamlit元件版)，
新增`_build_report_q3_html()`照抄desktop那份邏輯。兩邊都不重複帶「代號
名稱」當內部標題(改用外層`<h3>`當標題)，避免跟報表最上方的H1股票標籤
重複。

測試方式：桌面版用offscreen腳本對正式`data/tw_stock.db`的2330/2454/6182
三檔直接呼叫`_build_report_html()`驗證三維過濾法標題落在融資維持率分析
跟大戶籌碼中間。web版原本想直接Playwright點「產生PDF報表」按鈕驗證，
但本機Windows環境缺weasyprint需要的GTK/Pango原生函式庫(`libgobject-
2.0-0.dll`)裝不起來——跟本機無法連Turso是同一類「本機環境限制、不是
程式問題」的狀況，用`PYTHONPATH`塞一個假的`weasyprint`模組(把`HTML(
string=...).write_pdf()`攔截成直接把HTML寫進檔案，不做真的PDF轉檔)
繞過，讓「產生PDF報表」按鈕→`_build_full_report_html()`→組HTML這整條
真實程式路徑照樣完整跑一次，再檢查產出的HTML內容確認三維過濾法/融資
維持率分析/大戶籌碼三者順序正確。`pytest tests/ -q`1179個測試全過
(這次改動沒有新增獨立測試，兩個報表組裝函式本身沒有既有的單元測試
覆蓋，屬於既有的技術債，不在這次範圍內補)。
...
## 法人買賣總覽表格標示「累計」（2026-08-12）

使用者問「法人買賣總覽」表格(1日/2日/3日/5日/10日/20日/40日/3個月/
6個月/1年)是不是累計買賣超、5日>10日>40日代表什麼——解釋後（各欄是
往前累計、彼此重疊不是獨立區間，5日>10日>40日代表最近才開始加速買、
中期反而是賣）使用者確認這張表仍有價值(是預估持股成本的資料來源，見
`load_institutional_estimated_cost()`)，但覺得表格上標「累計」會更
清楚，避免使用者以為是不重疊的區間。

兩邊前端＋web版報表PDF三處(`desktop/main_window.py`的
`_build_overview_institutional_html()`、`dashboard/app.py`的互動版
expander跟報表版`_build_report_institutional_html()`)的「單位：張」
